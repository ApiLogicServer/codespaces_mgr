...
resourcename="carrental"  # lower case, only
resourcegroup="carrental_rg"
...
# git clone https://github.com/apilogicserver/carrental.git
# cd carrental
...
az group create --name carrental_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group carrental_rg --sku S1 --is-linux
az container create --resource-group carrental_rg --name carrental --image apilogicserver/carrental:latest --dns-name-label carrental --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//carrental.westus.azurecontainer.io:5656'
